...
func TestBimap(t *testing.T) {
	...
	s, ok := m.FindByInt(uint16(1))
	if !ok || s != "a" {
		t.Errorf("FindByStr(1) => %s, %v, want \"a\", true", s, ok)
	}

	err = m.Add("b", uint16(99))
	if err != nil {
		t.Errorf("Bimap.Add() failed with %v", err)
	}
	...
	_, ok = m.FindByInt(uint16(1))
	if ok {
		t.Errorf("Bimap.RemoveByStr(\"a\") didn't remove map entry")
	}

	err = m.RemoveByInt(uint16(99))
	if err != nil {
		t.Errorf("Bimap.RemoveByStr(99) => %v; want no error", err)
	}

	_, ok = m.FindByInt(uint16(99))
	if ok {
...
func TestGrowingBimap(t *testing.T) {
	m := New(2)
	for i := 0; i < 1000; i++ {
		err := m.Add(fmt.Sprintf("n:%d", i), uint16(i))
		if err != nil {
			t.Fatal(err)
		}
	}
